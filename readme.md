...
A pasta `metadata/` segue a estrutura padrão exportada pela CLI do Hasura (`version.yaml`, `databases/databases.yaml` e os arquivos em `databases/default/tables/`) e contém a configuração esperada para refletir as tabelas criadas pela migração:
